...
if [ "$FULL_CHECK" = true ]; then
    semgrep scan . --config auto --json > ./scanner/tmp/result/__semgrep__.json
else
    ...
    for file in $modified_files; do
        ...
        semgrep scan $file --config auto --json > "./scanner/tmp/result/__semgrep__${foldername}_${filename}.json"
    done
fi
